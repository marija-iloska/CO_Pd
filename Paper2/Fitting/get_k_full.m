function [k_oB, k_Bo, k_Ao, k_oA, k_AB, k_BA, k_oX, k_Xo, dlms] = get_k_full(cov, time, covA, covB, dt, tp_idx, tp_AB, N, P, M)
...
X = dt(tau)*P.*covB(tau);
% Y = covB(tau1) - covB(tau);
% Y = Y - k_AB*covA(tau).*dt(tau)*(M - cov)
% X = - dt(tau)*P.*covB(tau);
