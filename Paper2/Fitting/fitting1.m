...
for t = R1+2 : R2
    ...
    loss_B = - k_BA*dt(t)*theta_B(t-1); 

    theta_B(t) = theta_B(t-1) + gain_B + loss_B;
    
    % A
    %gain_A = k_oA*P*dt(t)*(M - covX) + k_BA*dt(t)*P*theta_B(t-1);
    gain_A = k_oA*P*dt(t)*(M - covX) + k_BA*dt(t)*theta_B(t-1);
...
theta_A(t) = covA(tp_idx-1);
theta_B(t) = covB(tp_idx-1);
...
for t = R2+1 : R3
    ...
    % B
    %gain_B = k_AB * dt(t) * theta_A(t - 1)*(M - covX);
    gain_B = k_AB * dt(t) * theta_A(t - 1)*(M - covX) - k_BA*dt(t)*theta_B(t-1);
...
for t = R3+1 : R4
    ...
    theta_B(t) = theta0*exp(-k_Bo*dtime(count));
    count = count+1;
end
